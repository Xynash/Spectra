...
import { Cpu, Target, Box, Code2, ShieldCheck, Terminal, Layout, Database, MousePointer2, Zap } from 'lucide-react';
...
export default function SpectraNode({ data }) {
  ...
  return (
    ...
    >
      ...
      {/* Click ripple ring — shows on hover for clickable nodes */}
      {isClickable && hovered && (
        <div className="absolute -inset-2 rounded-2xl border-2 border-dashed border-zinc-300 animate-pulse pointer-events-none" />
      )}
      ...
      <div className={`
        ...
        ${isRoot
          ...
          : 'border-zinc-200 group-hover:border-zinc-400/60'
        }
      ...
      `}>
        ...
        <div className="mt-4 pt-3 border-t border-zinc-100 flex items-center justify-between">
          ...
          {/* Click hint for non-root nodes */}
          {isClickable ? (
            <div className={`flex items-center gap-1 transition-opacity duration-200 ${hovered ? 'opacity-100' : 'opacity-0'}`}>
              <MousePointer2 size={10} className="text-zinc-400" />
              <span className="text-[8px] font-black text-zinc-400 uppercase tracking-wide">Explore</span>
            </div>
          ) : (
